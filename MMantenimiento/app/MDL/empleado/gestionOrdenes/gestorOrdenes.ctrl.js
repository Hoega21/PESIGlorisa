myMDLApp.controller("gestorOrdenesEmpCtrl", ['$scope', '$state', 'NgTableParams', '$location', '$http', '$cookies', '$stateParams',
function($scope, $state, NgTableParams, $location, $http, $cookies, $stateParams){
  ...
  ctrl.irCrearOrden = function () {
    $state.go('agregar-orden-e');
  }

  ctrl.irEditarOrden = function (orden) {
    $state.go('editar-orden-e', {id: orden.id});
  }

  ctrl.irInformeOrden = function (orden) {
    $state.go('informe-orden-e', {id: orden.id});
  }

  ctrl.regresarOrdenes = function () {
    $state.go('gestionar-ordenes-e');
  }
